...
class op(bpy.types.Operator):
	bl_idname = "uv.ttmini_color_from_elements"
...
def color_elements(self, context):
	...
	bpy.context.scene.ttmini_settings.color_ID_count = len(groups)
	gamma = 2.2

	for i in range(bpy.context.scene.ttmini_settings.color_ID_count):
		color = mini_color.get_color_id(i, bpy.context.scene.ttmini_settings.color_ID_count)
		# Fix Gamma
		color[0] = pow(color[0] , gamma)
		color[1] = pow(color[1] , gamma)
		color[2] = pow(color[2], gamma)
		mini_color.set_color(i, color)
	...
	for group in groups:
		...
		mini_color.assign_slot(obj, index_color)

		# Assign to selection
		obj.active_material_index = index_color
		bpy.ops.object.material_slot_assign()

		index_color = (index_color+1) % bpy.context.scene.ttmini_settings.color_ID_count

	bpy.ops.object.mode_set(mode='OBJECT')
	mini_color.validate_face_colors(obj)
