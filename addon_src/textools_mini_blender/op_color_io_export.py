...
class op(bpy.types.Operator):
	bl_idname = "uv.ttmini_color_io_export"
	bl_label = "Export"
...
def export_colors(self, context):
	...
	for i in range(tt_settings().color_ID_count):
		...
		hex_colors.append(mini_color.color_to_hex(color))

	bpy.context.window_manager.clipboard = ", ".join(hex_colors)
	bpy.ops.ui.ttmini_popup('INVOKE_DEFAULT', message=f"{len(hex_colors)}x colors copied to clipboard")
