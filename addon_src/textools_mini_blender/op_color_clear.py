...
class op(bpy.types.Operator):
	bl_idname = "uv.ttmini_color_clear"
...
def clear_colors(self, context):
	...
	if bpy.context.scene.ttmini_settings.color_assign_mode == 'MATERIALS':
		...
		for material in list(bpy.data.materials):
			if mini_color.material_prefix in material.name:
				if not material.users:
					bpy.data.materials.remove(material, do_unlink=True)
		bpy.ops.object.mode_set(mode=previous_mode)

	else:	#mode == VERTEXCOLORS
		mini_color.remove_color_layer(obj.data)
