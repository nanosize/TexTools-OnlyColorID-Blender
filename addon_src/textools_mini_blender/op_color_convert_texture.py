...
from . import mini_bake
from . import mini_color
from .settings import tt_settings


material_prefix = "TTMini_atlas_"
gamma = 2.2



class op(bpy.types.Operator):
	bl_idname = "uv.ttmini_color_convert_to_texture"
...
def pack_texture(self, context):
	...
	for c in range(tt_settings().color_ID_count):
		...
		color = list(mini_color.get_color(c))
		for i in range(3):
	...
	bpy.ops.object.mode_set(mode='OBJECT')
	bpy.ops.uv.ttmini_color_clear()
	bpy.ops.object.material_slot_add()
	
	#Create material with image
	obj.material_slots[0].material = mini_bake.get_image_material(image)
	...
	bpy.ops.ui.ttmini_popup('INVOKE_DEFAULT', message=f"Packed texture with {tt_settings().color_ID_count} color IDs")
